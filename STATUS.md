...
remaining:
  ...
  - tested criterion, Pickle green in both languages: NOT YET. Final passes of 2026-09-25 (tree 99be666, `-pickle-scenario-timeout=300`), 22 played of 22 in each: English 20 passed, French 21 passed (Evidence/2026-09-25-final-english and -final-french, docs/runs/2026-09-24-pickle-joy-route.md). The reds: `a bill in progress at the bench` in both (a fixed tick wait stopped short by the suite's own 90 s bound; fixed in 0a247d0) and `the uilleann pipes are played, and heard` in English (the save load timed out at 180 s, machine load). Replays on 0a247d0 requested (English bill ff21, French bill 063c, English pipes 2f06): the gate closes when all three are green
  ...
  - unverified: two runs (2026-09-22 8 of 8, 2026-09-25 request 17c3) failed on Pickle's own "Accessing map pawns off main thread" (the second in the built-in step `a colonist "Keeper" exists`, 82 ms into the first scenario); attributed to no mod, not explained; replay once and report to PickleTools if it repeats
  - tested criterion, no manual test left: MANUAL M1 (hear the three instruments, PLAY_AND_LISTEN.md) and MANUAL M2 (a real pre-split save, TEST_SCENARIOS.md) are to be done by the maintainer and reported; the accounting of every other scenario is in TEST_SCENARIOS.md
  ...
  - local state (2026-09-23): the mod and its tests are committed on main; origin/main = 166f297 (pushed 2026-09-22), local main is ahead by six commits including the 0.1.0 PublishedFileId commit, not pushed; the six .dds generated by the Workshop upload are ignored (.gitignore); run evidence lives on disk in Tests/Pickle/Evidence/ (ignored) with text summaries in docs/runs/; untracked and left alone on purpose: Tests/Audit-2026-09-13/ (decompilation evidence), Art/preview-background.png, Art/preview-render.png, Assets/
---
