...
remaining:
  - tested criterion, no @wip: met on paper (8 features, 22 scenarios, none tagged) but the reworked suite has never run; its 6 steps (Tests/Pickle/Source/InstrumentSteps.cs) have never been played in a game
  - tested criterion, conditional scenarios: none exists (no optional mod or DLC dependency, no @requires); nothing to run
  - tested criterion, Pickle green in both languages: queued 2026-09-23, English then French, 22 scenarios each; nothing yet. Read exitReason first, then 22 played of 22 written
  - unverified: the one Pickle run of the previous suite state (2026-09-22, 8 scenarios) failed 8 of 8 on an engine error attributed to no mod (Accessing map pawns off main thread; a Megascarab NullReferenceException); not reproduced, not explained; see the 2026-09-23 section
